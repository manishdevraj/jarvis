    ...
    private int findLASLengthRecursive(int[] nums, int previousIndex, int currentIndex, boolean isAsc) {
        ...
        if(isAsc) {
            if(previousIndex == -1 || nums[previousIndex] < nums[currentIndex])
                ascDescCount = 1 + this.findLASLengthRecursive(nums, currentIndex, currentIndex + 1, false);
        } else { // if descending, the next element should be smaller
            if(previousIndex == -1 || nums[previousIndex] > nums[currentIndex])
                ascDescCount = 1 + this.findLASLengthRecursive(nums, currentIndex, currentIndex + 1, true);
        }
        // skip the current element
        int skipCurrent = this.findLASLengthRecursive(nums, previousIndex, currentIndex + 1, isAsc);
        return Math.max(ascDescCount, skipCurrent);
    }
