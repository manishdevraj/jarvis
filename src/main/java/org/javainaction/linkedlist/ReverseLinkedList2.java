...
public class ReverseLinkedList2 {
    ...
    public static ListNode reverseBetween(ListNode head, int left, int right) {
        ...
        // 1 - 2 - 3 - 4 - 5 ; left = 2; right = 4 ---> prev = 1, current = 2, next = 3
        // dummy-> 1 -> 2 -> 3 -> 4 -> 5

        for (int i = 0 ; i < right - left; i++) {
            //move current to next's next 2 -> 4 in first and 2 -> 5 in second iteration
            current.next = next.next;
            //move next to prev's next 3 -> 2 in first and 4 -> 3 in second iteration
            next.next = prev.next;
            //now move prev to new next 1 -> 3 in first and 1 -> 4 in second iteration
            prev.next = next;
            //move next to new location 4 in first and 5 in second iteration
            next = current.next;
        }

        // first reversing : dummy->1 - 3 - 2 - 4 - 5; prev = 1, current = 2, next = 4
        // second reversing: dummy->1 - 4 - 3 - 2 - 5; prev = 1, current = 2, next = 5 (finish)
