...
public class NodesInSubtree {
    ...
    int[] countOfNodes(Node root, ArrayList<Query> queries, String s) {
        ...
        dfsBuildSubtreeMap(root, s, countMap);

        int index = 0;
        for (Query q : queries) {
            //fetch node value map's map that contains char frequency and return the how many nodes found frequency
            //as [U, C]
    ...
    /**
     *
     * Carefully observe the DFS solution it is trying to build a map of node value as a key and a map as value
     * where value map is all children's character under that node with a frequency
     *
