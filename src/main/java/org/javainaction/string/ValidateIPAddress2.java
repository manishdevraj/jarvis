...
/**
 * Given a string IP, return "IPv4" if IP is a valid IPv4 address, "IPv6" if IP is a valid IPv6 address or
...
public class ValidateIPAddress2 {
    public String validIPAddress(String ip) {
        ...
        if (ip == null || ip.length() == 0) return none;

        String[] ipv4 = ip.split("\\.");
        String[] ipv6 = ip.split("\\:");

        //if we have split string such that it can be made into valid IPv4 address then try for each token
        ...
        //if we have split string such that it can be made into valid IPv6 address then try for each token
