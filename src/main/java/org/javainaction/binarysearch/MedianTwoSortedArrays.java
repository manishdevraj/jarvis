...
public class MedianTwoSortedArrays {
     ...
     *
     * [2 3 / 5 7]
     * then the median = (3+5)/2. Note that I'll use '/' to represent a cut, and (number / number) to represent
     * a cut made through a number in this article.
     *
     ...
     * To get ready for the two array situation, let's add a few imaginary 'positions' (represented as #'s) in between
     * numbers, and treat numbers as 'positions' as well.
     *
     ...
     *
     * There are 2N1 + 2N2 + 2 position altogether.
     * Therefore, there must be exactly N1 + N2 positions on each side of the cut, and 2 positions directly on the cut.
     *
     ...
     * Now how do we decide if this cut is the cut we want? Because L1, L2 are the greatest numbers on the left halves
     * and R1, R2 are the smallest numbers on the right, we only need
     *
     ...
     *
     * If we have L1 > R2, it means there are too many large numbers on the left half of A1, then we must move C1
     * to the left (i.e. move C2 to the right);
     ...
     * then calculate the other accordingly. However, it is much more practical to move C2 (the one on the shorter
     * array) first. The reason is that on the shorter array, all positions are possible cut locations for median,
     * but on the longer array, the positions that are too far left or right are simply impossible for a legitimate cut.
     * For instance, [1], [2 3 4 5 6 7 8]. Clearly the cut between 2 and 3 is impossible, because the shorter array does
     * not have that many elements to balance out the [3 4 5 6 7 8] part if you make the cut this way.
     * Therefore, for the longer array to be used as the basis for the first cut, a range check must be performed.
     * It would be just easier to do it on the shorter array, which requires no checks whatsoever. Also,
     * moving only on the shorter array gives a run-time complexity of O(log(min(N1, N2)))
     *
     *
     ...
     * To solve this problem, we can imagine that both A1 and A2 actually have two extra elements, INT_MAX at A[-1]
     * and INT_MAX at A[N]. These additions don't change the result, but make the implementation easier:
     * If any L falls out of the left boundary of the array, then L = INT_MIN, and if any R falls out of the right
     * boundary, then R = INT_MAX.
     */
    ...
    public static void main(String[] args) {
        System.out.println("Median {1, 3} and {2} : " + findMedianSortedArrays(new int[]{1, 3}, new int[]{2}));
        System.out.println("Median {1, 2} and {3, 4} : " + findMedianSortedArrays(new int[]{1, 2}, new int[]{3, 4}));
        System.out.println("Median {} and {2} : " + findMedianSortedArrays(new int[]{}, new int[]{2}));
    }
}
