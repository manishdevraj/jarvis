...
public class RemoveIslands {
     ...
     * to such boarder 1's. If found mark 'true' in our auxillary data structure.
     *
     * @param matrix
     * @return
     */
    ...
    public static int[][] removeIslands(int[][] matrix) {
        ...
        for (int i = 0; i < matrix.length; i++) {
            for (int j = 0; j < matrix[i].length; j++) {
                ...
                //we need to only traverse an island at the borde
    ...
    public static void findOnesToBorder(int[][] matrix, int i, int j,
        ...
        if (i >=0 && i < matrix.length
                ...
                && !onesToBorder[i][j]
                && matrix[i][j] == 1) {
            //this will make sure we do not visit such nodes again
